...
**Deep-Link Scheme:** `theicecreamman://`

---

## Automated Release Workflow

Releases are now **fully automated** via GitHub Actions (`.github/workflows/eas-build-submit.yml`).
...
| `EAS_PROJECT_ID` | Run `npx eas init` in the repo, copy the UUID |
| `GOOGLE_SERVICE_ACCOUNT_KEY` | Google Play Console → Setup → API access → Service accounts (only needed for automated submit) |

### One-Time Setup: Link the EAS Project

```bash
# In the repo root — do this once:
npx eas init
# This prints your EAS_PROJECT_ID; add it as a GitHub secret.
```
...
1. Go to **Actions → EAS Build & Submit** in GitHub
