...
describe("Security Module - Client Side", () => {
  ...
  describe("sanitizeInput", () => {
    ...
    it("removes unquoted event handlers", () => {
      const input = "Photo onerror=alert(1) delivery note";
      const result = security.sanitizeInput(input);
    ...
    it("removes non-whitespace ASCII control characters", () => {
      const input = "Leave at gate\u0007\u001B please";
      const result = security.sanitizeInput(input);
      expect(result).toBe("Leave at gate please");
    });
