...
`app.config.ts` is the canonical Expo/EAS configuration. All identity constants are centralised in `config/app-identity.js` so they are shared across the app config, deep-link config, and tests. There is no `app.json` in this repo.

---

## 🔄 Release Workflow

The CI/CD pipeline lives in `.github/workflows/eas-build-submit.yml`:
...
| `GOOGLE_PLAY_SERVICE_ACCOUNT_JSON` | Google Play service-account JSON — required to auto-submit; omit to skip submission gracefully |
